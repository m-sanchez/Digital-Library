...
Monocle.Env = function() {
	...
	var envTests = [
		...
		//
		["touch", function() {
			result(Gala.Pointers.ENV.noMouse);
		}],
...
Monocle.CSS = function() {
	...
	function supportsProperty(props) {
		for (var i in props) {
			if (p.guineapig.style[props[i]] !== undefined) {
				return true;
			}
		}
		return false;
...
Gala.dispatch = function(elem, evtType, data, cancelable) {
	...
	if (elem.dispatchEvent) {
		...
		if (!Gala.IE_REGISTRATIONS[elem]) {
			return true;
		}
		var evtHandlers = Gala.IE_REGISTRATIONS[elem][evtType];
		if (!evtHandlers || evtHandlers.length < 1) {
			return true;
		}
		evt = {
			type: evtType,
...
Gala.replaceGroup = function(elem, listeners, newListeners, useCapture) {
	...
	for (var evtType in listeners) {
		delete listeners[evtType];
	}
	for (evtType in newListeners) {
		listeners[evtType] = newListeners[evtType];
	}
...
Gala.onTap = function(elem, fn, tapClass) {
	...
	if (typeof tapClass == 'undefined') {
		tapClass = Gala.TAPPING_CLASS;
	}
	...
	var fns = {
		start: function(evt) {
			...
			if (tapClass) {
				elem.classList.add(tapClass);
			}
		},
		move: function(evt) {
			if (!tapStartingCoords) {
				return;
			}
			if (!tapIsValid(evt)) {
				fns.cancel(evt);
		...
		cancel: function() {
			...
			if (tapClass) {
				elem.classList.remove(tapClass);
			}
		}
	};
	...
	var wrapContact = function(fn) {
		return function(evt) {
			if (Gala.Pointers.enabled()) {
				Gala.Pointers.trackPointers(evt);
			}
...
Gala.Cursor = function(evt) {
	...
	function initialize() {
		...
		while (tgt.nodeType != 1 && tgt.parentNode) {
			tgt = tgt.parentNode;
		}
...
Monocle.bookDataFromIds = function(elementIds) {
	return Monocle.bookData({
		...
		getComponent: function(cmptId) {
			return {
				nodes: [document.getElementById(cmptId)]
			}
		}
	});
}
...
Monocle.Events.afterTransition = function(elem, fn) {
	...
	cancel = function() {
		Monocle.Events.deafen(elem, evtName, l);
	}
...
Monocle.Formatting = function(reader, optStyles, optScale) {
	var API = {
		constructor: Monocle.Formatting
	};
	var k = API.constants = API.constructor;
	var p = API.properties = {
		reader: reader,
	...
	function adjustFontScaleForDoc(doc, scale) {
		if (scale) {
			...
			walkTree(doc.body, function(elem) {
				if (typeof elem.pfsOriginal == 'undefined') {
					return;
				}
	...
	function sweepElements(doc) {
		...
		walkTree(doc.body, function(elem) {
			...
			elem.pfsOriginal = fs;
			elem.pfsOriginalProp = elem.style ? elem.style.fontSize : '';
		});
...
Monocle.Reader = function(node, bookData, options, onLoadCallback) {
	...
	function createReaderElements() {
		...
		for (var i = 0; i < p.flipper.pageCount; ++i) {
			...
			page.m.multiColumn = page.m.sheafDiv.getBoundingClientRect().width > 1000;
			page.m.activeFrame = page.m.sheafDiv.dom.append('iframe', 'component', i);
			page.m.activeFrame.m = {
				'pageDiv': page
			};
	...
	function setBook(bk, place, callback) {
		...
		if (typeof callback == 'function') {
			...
			var listener = function(evt) {
				if (watchers[evt.type](evt)) {
					deafen(evt.type, listener);
				}
			}
			for (var evtType in watchers) {
				listen(evtType, listener)
			}
		}
		p.flipper.moveTo(place || {
			page: 1
		}, initialized);
	}
	...
	function onPageTurn(evt) {
		if (p.recalcPhase == 0) {
			...
			dispatchEvent('monocle:position', {
				place: place
			});
		}
	}
	...
	function moveTo(locus, callback) {
		...
		if (!locus.direction) {
			...
			dispatchEvent('monocle:jumping', {
				locus: locus
			});
			fn = function() {
				dispatchEvent('monocle:jump', {
					locus: locus
				});
				if (callback) {
					callback();
	...
	function addControl(ctrl, cType, options) {
		...
		var addControlTo = function(cntr) {
			if (cntr == 'container') {
				...
				if (!cntr.dom) {
					dom.claim(cntr, 'controlContainer');
				}
	...
	function hideControl(ctrl) {
		...
		dispatchEvent('monocle:controlhide', {
			control: ctrl
		}, false);
	}
	...
	function showControl(ctrl) {
		...
		dispatchEvent('monocle:controlshow', {
			control: ctrl
		}, false);
		return true;
	}
	...
	function fatalSystemMessage(msg) {
		var info = dom.make('div', 'book_fatality', {
			html: msg
		});
		var box = dom.find('box');
		var bbOrigin = [box.offsetWidth / 2, box.offsetHeight / 2];
		API.billboard.show(info, {
			closeButton: false,
			from: bbOrigin
		});
	}
...
Monocle.Book = function(dataSource, preloadWindow) {
	...
	function pageNumberAt(pageDiv, locus) {
		...
		if (typeof(locus.page) == "number") {
			...
			if(locus.multiColumn && result.page % 2 == 0 && !typeof(locus.addIfSameChapter) == "number"){
				result.page--;
			}
		} else if (typeof(locus.pagesBack) == "number") {
			result.page = lastPageNum + locus.pagesBack;
			if(locus.multiColumn && result.page % 2 == 0){
				result.page--;
			}
		...
		} else if (locus.addIfSameChapter) {
			result.page += locus.addIfSameChapter;
	...
	function componentIdMatching(str) {
		...
		for (var i = 0, ii = p.componentIds.length; i < ii; ++i) {
			if (decodeURIComponent(p.componentIds[i]) == str) {
				return str;
			}
		}
		return null;
...
Monocle.Place = function() {
	...
	function componentId() {
		return p.component.properties ? p.component.properties.id : undefined;
	}
	...
	function getLocus(options) {
		...
		if (options.direction) {
			locus.multiColumn = options.multiColumn;
			locus.page += options.direction;

			if(options.multiColumn){
				// We need to make it odd (page start at 1).
				if(locus.page % 2 == 0){
					locus.addIfSameChapter = options.direction;
				}
			}
		} else {
	...
	function percentageOfBook() {
		...
		for (var i = 0, ii = cmptIndex; i < ii; ++i) {
			pc += weights[i];
		}
...
Monocle.Place.percentOfBookToLocus = function(reader, percent) {
	...
	return {
		componentId: componentIds[cmptIndex],
		percent: cmptPercent
	}
};
/* COMPONENT */
...
Monocle.Component = function(book, id, index, chapters, source) {
			...
			(function(frame, src){
				frame.onload = function(){
					try {
						...
						// Keep styles added from monocle
						var styleElements = frame.contentDocument.head.getElementsByTagName("style");
						var styleElementsTxt = "";
						for(var i=0; i<styleElements.length; i++){
							styleElementsTxt += styleElements[i].outerHTML;
						}

		 				frame.contentDocument.head.innerHTML = doc.head.innerHTML + styleElementsTxt;
	...
	function pageForChapter(fragment, pageDiv) {
		...
		if(node.nodeName == "SECTION"){
			var subnode = null;
			for(var i=0; i<node.childNodes.length && !(subnode && subnode.getBoundingClientRect); i++){
...
Monocle.Billboard = function(reader) {
	...
	function show(urlOrElement, options) {
		...
		if (p.cntr) {
			return console.warn("Modal billboard already showing.");
		}
...
Monocle.Panels.IMode = function(flipper, evtCallbacks) {
	...
	function initialize() {
		...
		p.reader.addControl(p.toggleIcon, null, {
			hidden: true
		});
	}
...
Monocle.Panels.Magic = function(flipper, evtCallbacks) {
	...
	var API = {
		constructor: Monocle.Panels.Magic
	}
	var k = API.constants = API.constructor;
	var p = API.properties = {
		flipper: flipper,
	...
	function startListening() {
		if (p.disabled || p.startListeners.length) {
			return;
		}
...
Monocle.Flippers.Slider = function(reader) {
	...
	function moveTo(locus, callback) {
		...
		var page = upperPage();
		if(page.m.multiColumn)
			locus.multiColumn = true;
		
		setPage(page, locus, function() {
			prepareNextPage(cb)
		});
	}
	...
	function lift(dir, boxPointX) {
		if (p.turnData.lifting || p.turnData.releasing) {
			return;
		}
	...
	function onGoingBackward(x) {
		...
		if (Monocle.Browser.env.offscreenRenderingClipped) {
			...
			setPage(
				lp,
				getPlace(up).getLocus({
					direction: k.BACKWARDS
				}),
				function() {
			...
			jumpOut(lp, function() {
				...
				var locus = getPlace(up).getLocus({
					multiColumn: lp.m.multiColumn,
					direction: k.BACKWARDS
				});
	...
	function afterCancellingBackward() {
		...
		jumpIn(lowerPage(), function() {
			prepareNextPage(announceCancel);
		});
	}
	...
	function prepareNextPage(onLoad, onFail) {
		var page = lowerPage();
		var locus = getPlace().getLocus({
			multiColumn: page.m.multiColumn,
			direction: k.FORWARDS
		});
	...
	function jumpIn(pageDiv, callback) {
		var opts = {
			duration: (Monocle.Browser.env.stickySlideOut ? 1 : 0)
		}
	...
	function jumpOut(pageDiv, callback) {
		setX(pageDiv, 0 - pageDiv.offsetWidth, {
			duration: 0
		}, callback);
	}
	...
	function slideOpts() {
		...
		if (p.lastSlide && now - p.lastSlide < 1500) {
			opts.duration *= 0.5;
		}
		p.lastSlide = now;
		return opts;
