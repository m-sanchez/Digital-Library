...
EPUBJS.Book.prototype.getCurrentChapter = function(){
	var self = this;
	return self.getToc().then(function(toc){
		var label = null;
		var chapter = self.spine[self.spinePos];

		function tocContains(toc, cfi){
			if(toc.cfi == cfi){
				label = toc.label;
			}else if(toc.subitems){
				for(var i=0; i<toc.subitems.length && label == null; i++){
					tocContains(toc.subitems[i], cfi);
				}
			}
		}
		for(var i=0; i<toc.length && label == null; i++)
			tocContains(toc[i], chapter.cfi);
...
EPUBJS.core.request = function(url, type, withCredentials) {
	...
	function handler() {
		if (this.readyState === this.DONE) {
			if (this.status === 200 || (type == "xml" && this.responseXML) || (type == "binary" && typeof this.response == "object")){ // -- Firefox is reporting 0 for blob urls TODO json type
				var r;
...
EPUBJS.Render.Iframe.prototype.setStyle = function(style, val, prefixed){
	...
	if(this.bodyEl){
		if(!this.bodyEl.ePubStylesheet){
			var styleElm = this.document.createElement("style");
			styleElm.appendChild(this.document.createTextNode("")); // WebKit hack
			this.document.head.appendChild(styleElm);
			this.bodyEl.ePubStylesheet = styleElm.sheet;
		}
		
		var stylesheet = this.bodyEl.ePubStylesheet;
		addCSSRule(stylesheet, "*", style + ": " + val, stylesheet.rules.length);
	}
};

EPUBJS.Render.Iframe.prototype.removeStyle = function(style){
	if(this.bodyEl && this.bodyEl.ePubStylesheet){
		var stylesheet = this.bodyEl.ePubStylesheet;
		var rules = stylesheet.cssRules;

		var toDelete = [];
		for(var i=0; i<rules.length; i++){
			if(rules[i].style[style] != ""){
				toDelete.push(i);
			}
		}

		for(var i=toDelete.length-1; i >= 0; i--){
			stylesheet.deleteRule(toDelete[i]);
		}
	}
